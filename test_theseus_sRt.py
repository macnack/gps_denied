...
if __name__ == "__main__":
    ...
    H_1_2 = kornia.geometry.convert_affinematrix_to_homography(
        param_to_A(A4_1_2_tensor))
    ...
    H_hist_pre = torch.transpose(H_hist_pre['A4_1_2'][0, :, :], 1, 0)

    H_hist = kornia.geometry.convert_affinematrix_to_homography(
        param_to_A(H_hist_pre)).reshape(-1, 9)[:, :8].unsqueeze(0)

    H_hist = torch.transpose(H_hist, 1, 2)
    H_hist2 = {'H8_1_2': H_hist}

    print(f"Final Homography (H8): {H_hist.shape}")
    ...
    write_gif_batch(log_dir, feat1, feat2, H_hist2, H_rot, err_hist)
